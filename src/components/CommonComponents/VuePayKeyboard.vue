    ...
    <div class="pay-box" v-if="isPay">
        ...
        <div v-if="isPay">
            ...
            <div class="title v-1px-b pr">
                <span class="mint-button-icon" @click="close" :style="{float: 'left', marginLeft: '0.5rem'}"><i class="mintui mintui-back"></i></span>
                <span>{{payTitle}}</span>
            ...
            <div class="key-box" v-if="keyShow">
                <div class="item v-1px-t" v-for="(item, i) in keyList" :key="i">
                    <div class="key" v-for="(val, key) in item" :key="key" @touchstart="inputStart(val.val, $event)" @touchend="inputEnd($event)" :class="{'v-1px-l':key!=0}">
                        <!-- {{val}} -->
                        <img :src="val.src" :style="{width: '0.9rem'}"/>
                    </div>
                </div>
                <div class="item v-1px-t">
                    <div class="key  " style="background: #e8e8e8"></div>
                    <div class="key v-1px-l" @touchstart="inputStart(0, $event)" @touchend="inputEnd($event)">
                      <img src="./../../assets/0.png" :style="{width: '0.9rem'}"/>
                    </div>
                    <div class="key v-1px-l" style="background: #e8e8e8" @touchstart="del($event)" @touchend="inputEnd($event,'del')">
                      <img src="./../../assets/img_delete.png" :style="{width: '1.2rem'}"/>
                    </div>
                </div>
            </div>
...
export default {
  ...
  data () {
    return {
      ...
      val: [],
      keyList: [
        [{src: image1, val: 1}, {src: image2, val: 2}, {src: image3, val: 3}],
        [{src: image4, val: 4}, {src: image5, val: 5}, {src: image6, val: 6}],
        [{src: image7, val: 7}, {src: image8, val: 8}, {src: image9, val: 9}]
      ],
      payStatus: false
    }
  },
